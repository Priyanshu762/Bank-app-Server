...
import { withdraw } from '../services/operations/TransactionAPI'
...
const Withdraw = () => {
    const [formData, setFormData] = useState({
      ...
      const handleSubmit = async (e)=> {
          e.preventDefault()
          const response = await withdraw(type,amount,accountNumber)
  ...
  <form onSubmit={handleSubmit} className="flex flex-col items-center justify-center w-full max-w-md p-8 space-y-6 bg-white border-2 border-gray-300 rounded-lg shadow-md">
    <h2 className="text-2xl font-bold text-gray-700">Withdraw Form</h2>
    ...
    <button className="px-4 py-2 text-white bg-red-500 rounded-lg hover:bg-red-600 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-opacity-50" type="submit">Withdraw</button>
  </form>
</div>
